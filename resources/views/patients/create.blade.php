    ...
    <div class="col-12 col-lg-8">
        <div class="card p-4">
            <div class="mb-4">
                ...
                <h4 class="fw-bold mb-0">{{ __('messages.add_patient') }}</h4>
            ...
            <form action="{{ route('patients.store') }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" name="nik" id="nik" class="form-control @error('nik') is-invalid @enderror" value="{{ old('nik') }}" required placeholder="{{ __('messages.nik_placeholder') }}" maxlength="16">
                        @error('nik')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select name="gender" id="gender" class="form-select @error('gender') is-invalid @enderror" required>
                            <option value="" disabled selected>{{ __('messages.gender_placeholder') }}</option>
                            <option value="L" {{ old('gender') == 'L' ? 'selected' : '' }}>{{ __('messages.male') }}</option>
                            <option value="P" {{ old('gender') == 'P' ? 'selected' : '' }}>{{ __('messages.female') }}</option>
                        </select>
                        @error('gender')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="address" id="address" rows="3" class="form-control @error('address') is-invalid @enderror" placeholder="{{ __('messages.address_placeholder') }}">{{ old('address') }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-1"></i> {{ __('messages.save_patient') }}
                    </button>
                </div>
            </form>
        </div>
